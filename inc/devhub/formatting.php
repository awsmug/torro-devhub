 ...
 */
function torro_devhub_devhub_format_summary( $text ) {
	// Backticks in excerpts are not automatically wrapped in code tags, so do so.
	if ( false !== strpos( $text, '`' ) ) {
		$text = preg_replace_callback( '/`([^`]*)`/', 'torro_devhub_devhub_format_summary_replace_backticks_callback', $text );
	}
	...
	if ( false !== strpos( $text, '<' ) ) {
		$text = preg_replace_callback( '/(\s)(<[^ >]+>)(\s)/', 'torro_devhub_devhub_format_summary_replace_tags_callback', $text );
	}
 ...
 */
function torro_devhub_devhub_format_param_description( $text ) {
	...
	$text = torro_devhub_devhub_make_doclink_clickable( $text );
	$text = torro_devhub_devhub_fix_param_hash_formatting( $text );
 ...
 */
function torro_devhub_devhub_make_doclink_clickable( $content ) {
	if ( false === strpos( $content, '{@link ' ) && false === strpos( $content, '{@see ' ) ) {
		return $content;
	}

	return preg_replace_callback( '/\{@(?:link|see) ([^\}]+)\}/', 'torro_devhub_devhub_make_doclink_clickable_replace_callback', $content );
}

/**
 * Internal replace callback used by torro_devhub_devhub_format_summary().
 *
 * @since 1.0.0
 * @access private
 *
 ...
 */
function torro_devhub_devhub_format_summary_replace_backticks_callback( $matches ) {
 ...
 * @return string Replacement for the match.
 */
function torro_devhub_devhub_format_summary_replace_tags_callback( $matches ) {
	return $matches[1] . '<code>' . htmlentities( $matches[2] ) . '</code>' . $matches[3];
}

/**
 * Internal replace callback used by torro_devhub_devhub_make_doclink_clickable().
 *
 * @since 1.0.0
 * @access private
 *
 ...
 * @return string Replacement for the match.
 */
function torro_devhub_devhub_make_doclink_clickable_replace_callback( $matches ) {
	$link = $matches[1];
	...
	if ( 0 === strpos( $link, 'https://' ) || 0 === strpos( $link, 'http://' ) ) { // Link to an external resource.
		...
		$link = torro_devhub_devhub_resolve_internal_link( $link );
	}
 ...
 */
function torro_devhub_devhub_resolve_internal_link( $link ) {
	$post_types = array();
	$slug       = '';
	$url        = '';
	...
	if ( ! empty( $post_types ) && ! empty( $slug ) ) {
		 ...
		 */
		if ( apply_filters( 'torro_devhub_devhub_do_resolve_internal_links_strict', true ) ) {
			$url = torro_devhub_devhub_detect_internal_link_content( $slug, $post_types );
		} else {
 ...
 */
function torro_devhub_devhub_detect_internal_link_content( $slug, $post_types = array() ) {
 ...
 */
function torro_devhub_devhub_fix_param_hash_formatting( $text ) {
