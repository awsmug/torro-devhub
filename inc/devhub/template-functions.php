 ...
 */
function torro_devhub_devhub_get_summary( $post = null ) {
	$post = get_post( $post );

	$summary = $post->post_excerpt;

	if ( $summary ) {
		$summary = torro_devhub_devhub_format_summary( $summary );
 ...
 */
function torro_devhub_devhub_get_description( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_signature( $post = null ) {
	$post = get_post( $post );
	...
	if ( 'wp-parser-hook' === $post->post_type ) {
		...
		$hook_type = torro_devhub_devhub_get_hook_type( $post );
 ...
 */
function torro_devhub_devhub_get_params( $post = null ) {
	$post = get_post( $post );
	...
	if ( $tags ) {
		...
		foreach ( $tags as $tag ) {
			...
			if ( ! empty( $tag['name'] ) && 'param' === $tag['name'] ) {
				...
				$params[ $tag['variable'] ]['content'] = torro_devhub_devhub_format_param_description( $params[ $tag['variable'] ]['content'] );
			}
		}
	}
 ...
 */
function torro_devhub_devhub_get_arguments( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_return( $post = null ) {
	$post = get_post( $post );
	...
	$description = empty( $return['content'] ) ? '' : torro_devhub_devhub_format_param_description( $return['content'] );
 ...
 */
function torro_devhub_devhub_get_changelog( $post = null ) {
	$post = get_post( $post );
		...
		foreach ( $since_tags as $index => $meta ) {
			if ( is_array( $meta ) && $since_term->name === $meta['content'] ) {
				...
				if ( empty( $meta['description'] ) ) {
					if ( $deprecated ) {
						$description = torro_devhub_devhub_get_deprecated( $post, false );
					} elseif ( 0 === $index ) {
					...
					$description = '<span class="since-description">' . torro_devhub_devhub_format_param_description( $meta['description'] ) . '</span>';
				}
 ...
 */
function torro_devhub_devhub_get_see( $post = null ) {
	$post = get_post( $post );
	...
	foreach ( $tags as $tag ) {
		...
		$see_tags[] = array(
			'refers'  => torro_devhub_devhub_resolve_internal_link( $tag['refers'] ),
 ...
 */
function torro_devhub_devhub_get_deprecated( $post = null, $formatted = true ) {
	$post = get_post( $post );
	...
	if ( ! empty( $referral['refers'] ) ) {
		...
		if ( $refers ) {
			...
			$deprecation_info = sprintf( __( 'Use %s instead.', 'torro-devhub' ), torro_devhub_devhub_resolve_internal_link( $refers ) );
		}
	}
	...
	switch ( $type ) {
		case 'class':
			$contents = __( 'This class has been deprecated.', 'torro-devhub' );
			break;
		case 'trait':
			$contents = __( 'This trait has been deprecated.', 'torro-devhub' );
			break;
		case 'interface':
			$contents = __( 'This interface has been deprecated.', 'torro-devhub' );
			break;
		case 'function':
			$contents = __( 'This function has been deprecated.', 'torro-devhub' );
			break;
		case 'method':
			$contents = __( 'This method has been deprecated.', 'torro-devhub' );
			break;
		case 'hook':
			$contents = __( 'This hook has been deprecated.', 'torro-devhub' );
			break;
		default:
			$contents = __( 'This has been deprecated.', 'torro-devhub' );
	}
 ...
 */
function torro_devhub_devhub_get_private_access_message( $post = null ) {
	$post = get_post( $post );
	...
	if ( ! empty( $referral['refers'] ) ) {
		...
		if ( ! empty( $refers ) ) {
			...
			$alternative_string = sprintf( __( 'Use %s instead.', 'torro-devhub' ), torro_devhub_devhub_resolve_internal_link( $refers ) );
		}
	} else {
		$alternative_string = '';
	}

	$contents = __( 'This function&#8217;s access is marked private. This means it is not intended for use by other plugin or theme developers, only in this plugin itself. It is listed here for completeness.', 'torro-devhub' );
 ...
 */
function torro_devhub_devhub_get_uses( $post = null ) {
	...
	return get_posts( array(
		'post_type'           => torro_devhub_devhub_get_post_types(),
 ...
 * @return array Array of post objects.
 */
function torro_devhub_devhub_get_extends( $post = null ) {
	if ( ! function_exists( 'p2p_type' ) ) {
		return array();
	}
 ...
 * @return array Array of post objects.
 */
function torro_devhub_devhub_get_implements( $post = null ) {
	if ( ! function_exists( 'p2p_type' ) ) {
		return array();
	}
 ...
 * @return array Array of post objects.
 */
function torro_devhub_devhub_get_used_by( $post = null ) {
	if ( ! function_exists( 'p2p_type' ) ) {
		return array();
	}
 ...
 * @return array Array of post objects.
 */
function torro_devhub_devhub_get_extended_by( $post = null ) {
	if ( ! function_exists( 'p2p_type' ) ) {
		return array();
	}
 ...
 * @return array Array of post objects.
 */
function torro_devhub_devhub_get_implemented_by( $post = null ) {
	if ( ! function_exists( 'p2p_type' ) ) {
		return array();
	}
 ...
 * @return array Array of post objects.
 */
function torro_devhub_devhub_get_methods( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_hook_type( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_source_code( $post = null, $force_parse = false ) {
	$post = get_post( $post );
	...
	$source_file = torro_devhub_devhub_get_source_file( $post );
 ...
 */
function torro_devhub_devhub_get_source_file_link( $post = null, $line_number = true ) {
	...
	if ( $line_number ) {
		$line_number = torro_devhub_devhub_get_line_number( $post );
		if ( $line_number ) {
			$url .= '#L' . $line_number;
		}
	}
 ...
 */
function torro_devhub_devhub_get_source_file( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_line_number( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_source_file_archive_link( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_since_archive_link( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_package_archive_link( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_namespace_archive_link( $post = null ) {
	$post = get_post( $post );
 ...
 */
function torro_devhub_devhub_get_namespaces( $post = null, $sorted = false, $relative = false ) {
	$namespaces = wp_get_post_terms( $post->ID, 'wp-parser-namespace' );
 ...
 */
function torro_devhub_devhub_get_namespace( $post = null, $include_links = false, $relative = false ) {
	$post = get_post( $post );

	$namespaces = torro_devhub_devhub_get_namespaces( $post, true, $relative );
