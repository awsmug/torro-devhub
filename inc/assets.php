 ...
 * @since 1.0.0
 */
function torro_devhub_enqueue_assets() {
	$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

	wp_enqueue_style( 'torro-devhub-style', get_stylesheet_uri(), array(), TORRO_DEVHUB_VERSION );
	wp_style_add_data( 'torro-devhub-style', 'rtl', 'replace' );

	wp_enqueue_script( 'torro-devhub-script', get_theme_file_uri( '/assets/dist/js/theme' . $min . '.js' ), array(), TORRO_DEVHUB_VERSION, true );

	$theme_data = array();

	if ( has_nav_menu( 'primary' ) ) {
		$theme_data['navigation'] = array(
			'icon' => torro_devhub_get_svg( 'angle-down', array( 'fallback' => true ) ),
			'i18n' => array(
				'expand'   => __( 'Expand child menu', 'torro-devhub' ),
				'collapse' => __( 'Collapse child menu', 'torro-devhub' ),
			),
		);
	}
	...
	if ( is_singular() && comments_open() ) {
		$theme_data['comments'] = array(
			'i18n' => array(
				'processing'    => __( 'Processing...', 'torro-devhub' ),
				'badResponse'   => __( 'Bad response code.', 'torro-devhub' ),
				'invalidJson'   => __( 'Invalid JSON response.', 'torro-devhub' ),
				/* translators: %s: edit comment URL */
				'flood'         => sprintf( __( 'Your comment was either a duplicate or you are posting too rapidly. <a href="%s">Edit your comment</a>', 'torro-devhub' ), '#comment' ),
				'error'         => __( 'There were errors in submitting your comment; complete the missing fields and try again!', 'torro-devhub' ),
				'emailInvalid'  => __( 'This email address appears to be invalid.', 'torro-devhub' ),
				'required'      => __( 'This is a required field.', 'torro-devhub' ),
				'commentsTitle' => sprintf(
					/* translators: 1: title. */
					__( 'One thought on &ldquo;%1$s&rdquo;', 'torro-devhub' ),
	...
	if ( ! empty( $theme_data ) ) {
		wp_localize_script( 'torro-devhub-script', 'themeData', $theme_data );
	}
}
add_action( 'wp_enqueue_scripts', 'torro_devhub_enqueue_assets' );
 ...
 */
function torro_devhub_handle_ajax_comment( $id, $status ) {
	...
	switch ( $status ) {
		...
		case '1':
			...
			$comment_class = comment_class( 'torro-devhub-ajax-comment', $comment->comment_ID, $comment->comment_post_ID, false );

			$args = torro_devhub_get_list_comments_args();
			$args['walker'] = null;

			ob_start();
			torro_devhub_render_comment( $comment, $args, 0, true );
			...
			if ( '0' === (string) $status ) {
				$message = __( 'Thanks for commenting! Your comment has been sent for moderation and should be approved soon.', 'torro-devhub' );
			} else {
				/* translators: %s: comment URL */
				$message = sprintf( __( 'Thanks for commenting! Your comment has been approved. <a href="%s">Read your comment</a>', 'torro-devhub' ), "#comment-$id" );
			}
		...
		case 'spam':
			wp_send_json( array(
				'response' => '',
				'success'  => 0,
				'status'   => __( 'Your comment has been marked as spam.', 'torro-devhub' ),
			), 200 );
			break;
		default:
			wp_send_json( array(
				'response' => '',
				'success'  => 0,
				'status'   => __( 'An unknown error occurred while trying to post your comment.', 'torro-devhub' ),
			), 200 );
	}

	exit;
}
add_action( 'comment_post', 'torro_devhub_handle_ajax_comment', 20, 2 );
 ...
 */
function torro_devhub_render_comment( $comment, $args, $depth = 0, $close = false ) {
 ...
 */
function torro_devhub_get_list_comments_args() {
	/**
	 ...
	 */
	return apply_filters( 'torro_devhub_list_comments_args', array(
		'walker'      => new Walker_Comment(),
		'max_depth'   => '',
		'style'       => 'ol',
		'callback'    => 'torro_devhub_render_comment',
		'avatar_size' => 32,
		'short_ping'  => true,
	) );
}
 ...
 * @since 1.0.0
 */
function torro_devhub_include_svg_icons() {
...
add_action( 'wp_footer', 'torro_devhub_include_svg_icons', 9999 );
 ...
 */
function torro_devhub_get_svg( $icon, $args = array() ) {
 ...
 */
function torro_devhub_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	$social_icons = torro_devhub_get_social_links_icons();

	if ( 'social' === $args->theme_location ) {
		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace( $args->link_after, '</span>' . torro_devhub_get_svg( $value ), $item_output );
			}
		}
	}
...
add_filter( 'walker_nav_menu_start_el', 'torro_devhub_nav_menu_social_icons', 10, 4 );
 ...
 */
function torro_devhub_dropdown_icon_to_menu_link( $title, $item, $args ) {
	if ( 'primary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title .= torro_devhub_get_svg( 'angle-down' );
				break;
			}
		}
	}
...
add_filter( 'nav_menu_item_title', 'torro_devhub_dropdown_icon_to_menu_link', 10, 3 );
 ...
 */
function torro_devhub_get_social_links_icons() {
	 ...
	 */
	return apply_filters( 'torro_devhub_social_links_icons', $social_links_icons );
}
